...
func TestTools_UploadFiles(t *testing.T) {
	for _, e := range uploadTests {
		...
		//read from the pipe which recives data
...
var jsonTests = []struct {
	...
	{name: "two json file", json: `{"foo":"1"}{"alpha":"beta"}`, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "empty", json: ``, errorExpected: true, maxSize: 1024, allowUnknown: false},
